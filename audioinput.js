...
class audioinputNode extends AudioWorkletNode {

    constructor(context, baseURL, options) {
        super(context, 'audioinput', options);
    ...
    ctrlChange(channel, ctrl, value) {
        if (this.fCtrlLabel[ctrl] !== []) {
            for (var i = 0; i < this.fCtrlLabel[ctrl].length; i++) {
                ...
                this.setParamValue(path, audioinputNode.remap(value, 0, 127, this.fCtrlLabel[ctrl][i].min, this.fCtrlLabel[ctrl][i].max));
                if (this.output_handler) {
    ...
    pitchWheel(channel, wheel) {
        for (var i = 0; i < this.fPitchwheelLabel.length; i++) {
            ...
            this.setParamValue(pw.path, audioinputNode.remap(wheel, 0, 16383, pw.min, pw.max));
            if (this.output_handler) {
...
class audioinput {
    ...
    async load() {
        try {
            ...
            let real_url = (this.baseURL === "") ? "audioinput.wasm" : (this.baseURL + "/audioinput.wasm");
            ...
            if (this.fWorkletProcessors.indexOf(name) === -1) {
                try {
                    let re = /JSON_STR/g;
                    let audioinputProcessorString1 = audioinputProcessorString.replace(re, json);
                    let real_url = window.URL.createObjectURL(new Blob([audioinputProcessorString1], { type: 'text/javascript' }));
            ...
            this.node = new audioinputNode(this.context, this.baseURL,
                {
            ...
            this.node.onprocessorerror = () => { console.log('An error from audioinput-processor was detected.'); }
            return (this.node);
        } catch (e) {
            console.error(e);
            console.error("Faust " + this.name + " cannot be loaded or compiled");
            ...
            try {
                ...
                if (!this.linkExists(real_url)) {
                    ...
                    link.onload = (e) => {
                        ...
                        var element = createaudioinputGUI(this.node);
                        resolve(element);
                    }
                } else {
...
let audioinputProcessorString = `
    ...
    class audioinputProcessor extends AudioWorkletProcessor {
        ...
        static get parameterDescriptors() 
        {
            ...
            var params = [];
            audioinputProcessor.parse_ui(JSON.parse(\`JSON_STR\`).ui, params, audioinputProcessor.parse_item1);
            return params;
        }
...
const dspName = "audioinput";

// WAP factory or npm package module
if (typeof module === "undefined") {
    window.audioinput = audioinput;
    window.Faustaudioinput = audioinput;
    window[dspName] = audioinput;
} else {
